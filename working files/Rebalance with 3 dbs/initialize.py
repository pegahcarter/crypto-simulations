def Initialize(db1, db2, db3):
	def initialize_csv():
		if Path(os.getcwd() + '/dbs/csv/transactions.csv') or not db1:
			return

		portfolio = []
		transactions = []
		for trade_id, coin in enumerate(coins):
			price, quantity, dollar_value = pull_coin_info(coin)
			transactions.append([
				trade_id,				# trade_id
				0, 						# rebalance_id
				str(datetime.now()),	# date
# 				coin,					# coin
				'buy', 					# side
				coin + '/USD',			# ratio
				price, 					# price
				quantity, 				# quantity
				dollar_value, 			# dollar_value
				dollar_value * 0.0075	# fees
			])



		transactions_pd = pd.DataFrame(
			transactions,
			columns = [
				'trade_id',
				'rebalance_id',
				'date',
				'coin',
				'side',
				'ratio',
				'price',
				'quantity'
				'dollar_value',
				'fees'
			]
		)

		transactions_pd.to_csv('/dbs/csv/transactions.csv')
'''
		portfolio.append([
			coin,					# coin
			price,					# current_price
			quantity,				# quantity
			dollar_value			# dollar_value
		])

		portfolio_pd = pd.DataFrame(
			portfolio,
			columns =
			[
				'coin',
				'current_price',
				'quantity',
				'dollar_value'
			]
		)
		portfolio_pd.to_csv('/dbs/csv/portfolio.csv')
'''
	# End of initialize_csv function

	def initialize_sql():
		if Path(os.getcwd() + '/dbs/sql/rebalance.db') or not db2:
			return

		import dbs.sql.setup
		engine = create_engine('sqlite:///dbs/sql/rebalance.db')
		Base.metadata.bind = engine
		DBSession = sessionmaker(bind=engine)
		session = DBSession()

		for trade_id, coin in enumerate(coins):
			price, quantity, dollar_value = pull_coin_info(coin)

			session.add(Transactions(
				trade_id = trade_id,
				rebalance_id = 0,
				date = str(datetime.now()),
# 				coin = coin,
				side = 'buy',
				ratio = coin + '/USD',
				price = price,
				quantity = quantity,
				dollar_value = dollar_value
				fees = dollar_value * 0.0075
			))
			session.commit()

'''
			session.add(Portfolio(
				coin = coin,
				current_price = price,
				quantity = quantity,
				dollar_value = dollar_value
				))
			session.commit()
'''
	# End of initialize_sql function

	def initialize_mongo():
		# Note: change to default mongodb save location instead
		if Path(os.getcwd() + '/dbs/mongo/rebalance.db') or not db3:
			return

		client = MongoClient()
		mongo_db = client.transactions
		for trade_id, coin in enumerate(coins):
			price, quantity, dollar_value = pull_coin_info(coins)
			mongo_db.insert_one({
				'trade_id': trade_id,
				'rebalance_id': 0,
				'date': str(datetime.now()),
				'side': 'buy',
				'ratio': coin + '/USD',
				'price': price,
				'quantity': quantity,
				'dollar_value': dollar_value,
				'fees': dollar_value * 0.0075
			})
	# End of initialize_mongo function

	initialize_csv()
	initialize_sql()
	initialize_mongo()

if __name__ = '__main__':
	sys.exit('Error: do not run this program separately from rebalace.py.')
